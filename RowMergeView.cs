...
using System.Runtime.InteropServices;
    ...
    public partial class RowMergeView : DataGridView
    {
        ...
        protected override void OnCellClick(DataGridViewCellEventArgs e)
        {
            if (!this.Key.Equals(""))
            {
                if (e.RowIndex >= 0)
                {
                    ...
                    {
                        if (this.Rows[i].Cells[this.Key].Value.ToString().Equals(this.Rows[e.RowIndex].Cells[this.Key].Value.ToString()))
                        {
                            foreach (string strTemp in this.MergeColumnNames)
                            {
                                this.Rows[i].Cells[strTemp].Selected = this.Rows[e.RowIndex].Cells[strTemp].Selected;
                            }
                        }
                        else
                        {
        ...
        {
            MergeRowIndex.Remove(e.RowIndex);

            if (!this.Key.Equals(""))
            {
                if (e.RowIndex == 0)
                {
                    MergeRowIndex.Add(e.RowIndex, 1);
                }
                else
                {
                    try
                    {
                        int itmp = MergeRowIndex[e.RowIndex - 1];
                    }
                    catch
                    {
                        for (int i = 1; i < e.RowIndex; i++)
                        {
                            if (this.Rows[i].Cells[this.Key].Value.ToString().Equals(this.Rows[i - 1].Cells[this.Key].Value.ToString()))
                            {
                                MergeRowIndex.Remove(i);
                                MergeRowIndex.Add(i, MergeRowIndex[i - 1]);
                            }
                            else
                            {
                                MergeRowIndex.Remove(i);
                                MergeRowIndex.Add(i, MergeRowIndex[i - 1] + 1);
                            }
                        }
                    }
                    finally
                    {
                        if (this.Rows[e.RowIndex].Cells[this.Key].Value.ToString().Equals(this.Rows[e.RowIndex - 1].Cells[this.Key].Value.ToString()))
                        {
                            MergeRowIndex.Add(e.RowIndex, MergeRowIndex[e.RowIndex - 1]);
                        }
                        else
                        {
                            MergeRowIndex.Add(e.RowIndex, MergeRowIndex[e.RowIndex - 1] + 1);
                        }
            ...
            {
                MergeRowIndex.Add(e.RowIndex, e.RowIndex + 1);
            }
        ...
        private void DrawCell(DataGridViewCellPaintingEventArgs e)
        {
            ...
            if (this.MergeColumnNames.Contains(this.Columns[e.ColumnIndex].Name) && e.RowIndex != -1)
            {
                ...
                //bool bFlag = false;
                //foreach ( DataGridViewCell tmp  in this.SelectedCells)
                //{
                //    if(this.Rows[e.RowIndex].Cells.Contains(tmp))
                //    {
                //        bFlag = true;
                //        break;
                //    }
                ...
                //if (this.SelectedCells.Count > 0 && bFlag)
                ...
                //画字符串
                PaintingFont(e, cellwidth, UpRows, DownRows, count, fontBrush.Color);
            ...
            if (this.NoLink.Contains(this.Rows[e.RowIndex].Cells[e.ColumnIndex]) && e.RowIndex != -1)
            {
                cellwidth = e.CellBounds.Width;
                Pen gridLinePen = new Pen(gridBrush);
                ...
                e.Graphics.FillRectangle(backBrush, e.CellBounds);
                //画字符串
                PaintingFont(e, cellwidth, 1, 1, 1, fontBrush.Color,true);
        ...
        private void PaintingFont(System.Windows.Forms.DataGridViewCellPaintingEventArgs e, int cellwidth, int UpRows, int DownRows, int count,Color ForeColor,bool isNoLink = false)
        {
            ...
            string strOut = e.Value.ToString();

            Font tmpFont;

            if (!isNoLink)
            {
                if (this.Columns[e.ColumnIndex] is DataGridViewLinkColumn)
                {
                    fontBrush = new SolidBrush(Color.Blue);

                    tmpFont = new Font(e.CellStyle.Font.FontFamily.Name, e.CellStyle.Font.Size, FontStyle.Underline);
                }
                else
                {
                    tmpFont = new Font(e.CellStyle.Font.FontFamily.Name, e.CellStyle.Font.Size, e.CellStyle.Font.Style);
                }
            }
            else
            {
            ...
            bool flag = false;
            while (fontwidth > cellwidth)
            {
                strOut = strOut.Substring(0, strOut.Length - 2);
                fontwidth = (int)e.Graphics.MeasureString(strOut, e.CellStyle.Font).Width;
                flag = true;
            }
            ...
            {
                e.Graphics.DrawString((String)strOut, tmpFont, fontBrush, e.CellBounds.X + (cellwidth - fontwidth) / 2, e.CellBounds.Y + cellheight * DownRows - fontheight);
            }
            else if (e.CellStyle.Alignment == DataGridViewContentAlignment.BottomLeft)
            {
                e.Graphics.DrawString((String)strOut, tmpFont, fontBrush, e.CellBounds.X, e.CellBounds.Y + cellheight * DownRows - fontheight);
            }
            else if (e.CellStyle.Alignment == DataGridViewContentAlignment.BottomRight)
            {
                e.Graphics.DrawString((String)strOut, tmpFont, fontBrush, e.CellBounds.X + cellwidth - fontwidth, e.CellBounds.Y + cellheight * DownRows - fontheight);
            }
            else if (e.CellStyle.Alignment == DataGridViewContentAlignment.MiddleCenter)
            {
                e.Graphics.DrawString((String)strOut, tmpFont, fontBrush, e.CellBounds.X + (cellwidth - fontwidth) / 2, e.CellBounds.Y - cellheight * (UpRows - 1) + (cellheight * count - fontheight) / 2);
            }
            else if (e.CellStyle.Alignment == DataGridViewContentAlignment.MiddleLeft)
            {
                e.Graphics.DrawString((String)strOut, tmpFont, fontBrush, e.CellBounds.X, e.CellBounds.Y - cellheight * (UpRows - 1) + (cellheight * count - fontheight) / 2);
            }
            else if (e.CellStyle.Alignment == DataGridViewContentAlignment.MiddleRight)
            {
                e.Graphics.DrawString((String)strOut, tmpFont, fontBrush, e.CellBounds.X + cellwidth - fontwidth, e.CellBounds.Y - cellheight * (UpRows - 1) + (cellheight * count - fontheight) / 2);
            }
            else if (e.CellStyle.Alignment == DataGridViewContentAlignment.TopCenter)
            {
                e.Graphics.DrawString((String)strOut, tmpFont, fontBrush, e.CellBounds.X + (cellwidth - fontwidth) / 2, e.CellBounds.Y - cellheight * (UpRows - 1));
            }
            else if (e.CellStyle.Alignment == DataGridViewContentAlignment.TopLeft)
            {
                e.Graphics.DrawString((String)strOut, tmpFont, fontBrush, e.CellBounds.X, e.CellBounds.Y - cellheight * (UpRows - 1));
            }
            else if (e.CellStyle.Alignment == DataGridViewContentAlignment.TopRight)
            {
                e.Graphics.DrawString((String)strOut, tmpFont, fontBrush, e.CellBounds.X + cellwidth - fontwidth, e.CellBounds.Y - cellheight * (UpRows - 1));
            }
            else
            {
                e.Graphics.DrawString((String)strOut, tmpFont, fontBrush, e.CellBounds.X + (cellwidth - fontwidth) / 2, e.CellBounds.Y - cellheight * (UpRows - 1) + (cellheight * count - fontheight) / 2);
        ...
        private List<object> _noLink = new List<object>();

        public System.Collections.Generic.Dictionary<int, int> MergeRowIndex
        {
            get
            {
                return _mergeRowIndex;
            }
            set
            {
                _mergeRowIndex = value;
            }
        }

        private System.Collections.Generic.Dictionary<int,int> _mergeRowIndex = new Dictionary<int,int>();
        #endregion
        #region 二维表头
